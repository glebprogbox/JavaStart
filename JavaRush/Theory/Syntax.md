...
## Команды
делятся на 
- команды, сделать что-то, int x;
- выражения, вычислить что-то x = 3+a;
...
Переменная стринги занимает 4 байта и хранит ссылку на объект типа String
Если String text = abc; то будет так 
4 байта abc - ссылка на блок памяти с текстом abc
4 байта text - ссылка на тот же блок памяти с текстом abc

При сравнении сравнивается адрес ссылки! Важно)) Не "начинка", не текст
А для одинаковых значений компилятор использует один и тот же блок памяти, то есть ссылки у String a ="kart" и String b = "kart" будут одинаковыми. если важно, чтобы  объект был новым, надо написать String b = new String (a); (например)
Все строки хранятся в String Pool (одно место, где все строчки).
new String создает строку вне этого пула, остальные же будут там, в пуле. Исключение - одинаковые строки все таки могут быть в пуле, тк строка  может быть добавлена в него JVM в результате string interning.

Чтобы сравнивать текст, будем использовать equals
строка.equals(строка2) - вернется тру или фолс
можно сравнивать без учета регистра, equalsIgnoreCase
...
Выражение 1 выполняется если тру, выражение 2 если фолс
пример
int a =2;
int b =3;
int min = a<b?a:b; // будет a, тк a<b тру, в итоге min = a; выражение 1

int age = 25;
int money = age>30?100:50; // будет 50, тк age <30, фолс, выражение 2

## Math.abs - модуль
Вещественные числа лучше сравнивать с точностью
int a = 0,01321....
int b = 0,01331....
if(Math.abs(a-b)<0.001); // а от б отличается на 0.001 или нет?
или
boolean done = (Math.abs(a-b)<0,001);  // а от б отличается на 0.001 или нет?
